...
public class EntityMechanicalArm extends Entity {
	...
	public EntityMechanicalArm(World world, double i, double j, double k,
			double width, double height, TileQuarry parent) {
		this(world);
		setPositionAndRotation(parent.xCoord, parent.yCoord, parent.zCoord, 0,
				0);
		this.xRoot = i;
		this.yRoot = j;
		this.zRoot = k;
	...
	private void findAndJoinQuarry() {
		TileEntity te = worldObj.getBlockTileEntity((int) posX, (int) posY,
				(int) posZ);
	...
	private double[] getHead() {
		return new double[] { this.headX / 32D, this.headY / 32D,
				this.headZ / 32D };
	}
}
