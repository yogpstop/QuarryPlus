...
public class TilePump extends APacketTile implements IFluidHandler, IEnchantableTile,
  ...
  protected void C_recievePacket(final byte id, final byte[] data, final EntityPlayer ep) {
    ...
    switch (id) {
      ...
      case PacketHandler.StC_OPENGUI_MAPPING:// BI[Ljava.lang.String;
        ...
        for (int i = 0; i < len; i++)
          this.mapping[target].add(badi.readUTF());
        ep.openGui(QuarryPlus.instance, QuarryPlus.guiIdPump + target, this.worldObj, this.xCoord,
            this.yCoord, this.zCoord);
        break;
    }
  }

  void S_OpenGUI(final int d, final EntityPlayer ep) {// BI[Ljava.lang.String;
    try {
  ...
  void S_changeRange(final EntityPlayer ep) {
