...
public class RRNew implements IFlexibleCrafter {
  private final TileRefinery tile;

  private RRNew(final TileRefinery tr) {
    this.tile = tr;
  }
  ...
  static void get(final TileRefinery tr) {
    ...
    for (final IFlexibleRecipe<FluidStack> ifr : irrm.getRecipes()) {
      final CraftingResult<FluidStack> cr = ifr.craft(new RRNew(tr), true);
      if (cr == null || !RefineryRecipeHelper.check(cr.crafted, tr))
        continue;
      ifr.craft(new RRNew(tr), false);
