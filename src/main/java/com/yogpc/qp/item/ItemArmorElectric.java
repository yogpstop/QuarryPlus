...
public class ItemArmorElectric extends ItemArmor implements ISpecialArmor, IElectricItem,
  ...
  public ItemArmorElectric() {
    super(EnumHelper.addArmorMaterial("ELECTRIC", 33, new int[] {3, 8, 6, 3}, 0), QuarryPlus.proxy
  ...
  private void useJetpack(final EntityPlayer p, final ItemStack jetpack, final boolean hover) {
    final boolean jumping = QuarryPlus.proxy.getKey(p, ProxyCommon.Key.jump);
  ...
  private static boolean toggleHover(final EntityPlayer p, final ItemStack itemStack) {
    ...
    if (QuarryPlus.proxy.getKey(p, ProxyCommon.Key.jump)
        && QuarryPlus.proxy.getKey(p, ProxyCommon.Key.mode) && toggleTimer == 0) {
      toggleTimer = 10;
      hoverMode = !hoverMode;
