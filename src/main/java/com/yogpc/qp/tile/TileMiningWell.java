...
public class TileMiningWell extends TileBasic {
  ...
  public void updateEntity() {
    ...
    while (!S_checkTarget(depth)) {
      if (this.working)
        this.worldObj.setBlock(this.xCoord, depth, this.zCoord, QuarryPlusI.blockPlainPipe);
      depth--;
    }
    if (this.working)
  ...
  private boolean S_checkTarget(final int depth) {
    ...
    if (b == null || h < 0 || b == QuarryPlusI.blockPlainPipe
  ...
  protected void G_destroy() {
    ...
    for (int depth = this.yCoord - 1; depth > 0; depth--) {
      if (this.worldObj.getBlock(this.xCoord, depth, this.zCoord) != QuarryPlusI.blockPlainPipe)
        break;
